...
def ingest_data(source_table, temp_storage):
    ...
    # result, columns = print_query(conn, f"SELECT * FROM {source_table} WHERE updated_at >= '{yesterday} 00:00:00' AND created_at <= '{yesterday} 23:59:59';") # Jangan lupa untuk where h-1

    today = (datetime.now()).strftime("%Y-%m-%d")

    result, columns = print_query(conn, f"SELECT * FROM {source_table} WHERE created_at <= '{today}';") # Jangan lupa untuk where h-1
